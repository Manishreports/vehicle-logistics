...
import { parseBulkPaste, type BulkKind, type BulkParseResult } from '../services/bulkPasteParser';
...
function App() {
  ...
  function importBulk(kind: BulkKind, result: BulkParseResult) {
    const valid = result.rows.filter((row) => row.status === 'valid').map((row) => row.record);
    if (!valid.length) return setToast('No valid rows to import.');
    const timestamp = now();
    if (kind === 'planning') {
      const records = valid as PlanningRecord[];
      const imported = records.map((record) => ({ ...record, id: rid(), createdAt: timestamp, updatedAt: timestamp, source: 'import' as const }));
      setState((s) => ({ ...s, planningRecords: [...s.planningRecords, ...imported] }));
    } else {
      const records = valid as StatusRecord[];
      const imported = records.map((record) => ({ ...record, id: rid(), createdAt: timestamp, updatedAt: timestamp }));
      setState((s) => ({ ...s, statusRecords: [...s.statusRecords, ...imported] }));
    }
    setModal(null);
    setToast(`${valid.length} records imported successfully.`);
  }
  function addRaipur() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {page === 'planning' && <PlanningPage groups={filteredPlanning} kpi={kpi1} search={planningSearch} setSearch={setPlanningSearch} filter={planningFilter} setFilter={setPlanningFilter} onAdd={() => { setDraft({}); setModal('planning-add'); }} onBulk={() => setModal('bulk-planning')} onEdit={(g) => { setDraft(g.records[0] as any); setModal(`planning-edit:${g.records[0].id}`); }} onDelete={(g) => confirmDelete('planningRecords', g.records[0].id)} />}
      {page === 'status' && <StatusPage groups={filteredStatus} kpi={kpi2} search={statusSearch} setSearch={setStatusSearch} filter={statusFilter} setFilter={setStatusFilter} onNavigate={(filter: string) => filter === 'Vehicle Call Pending' ? navTo('planning', 'Vehicle Call Pending') : navTo('status', filter === 'Vehicle called, Plan Pending' ? 'Vehicle called, Plan Pending' : filter)} onAdd={() => { setDraft({}); setModal('status-add'); }} onBulk={() => setModal('bulk-status')} onDelete={(g) => confirmDelete('statusRecords', g.records[0].id)} />}
    ...
    {modal && <Modal title={modal.startsWith('planning') ? 'Vehicle Planning' : modal.startsWith('status') ? 'Vehicle Call' : modal.startsWith('raipur') ? 'Raipur Dataset' : modal === 'import' ? 'Combined Vehicle In / Vehicle Out Excel' : 'Manual Mapping'} onClose={() => setModal(null)}>
      ...
      {modal === 'bulk-planning' && <BulkPasteModal kind="planning" state={state} mappings={state.mappings} onImport={(result) => importBulk('planning', result)} onClose={() => setModal(null)} />}
      {modal === 'bulk-status' && <BulkPasteModal kind="status" state={state} mappings={state.mappings} onImport={(result) => importBulk('status', result)} onClose={() => setModal(null)} />}
...
function BulkPasteModal({ kind, state, mappings, onImport, onClose }: { kind: BulkKind; state: AppState; mappings: MappingRule[]; onImport: (result: BulkParseResult) => void; onClose: () => void }) {
  const [text, setText] = useState('');
  const preview = useMemo(() => parseBulkPaste(kind, text, state.planningRecords, state.statusRecords, mappings, () => 'preview', now), [kind, text, state.planningRecords, state.statusRecords, mappings]);
  const headers = preview.headers;
  const schemaKeys = kind === 'planning'
    ? ['date','location','plant','cfa','weight','sto','loadingPoint','vehicleIn','vehicleNumber','vehicleOut','slipNumber']
    : ['demandDate','requiredDate','location','loadingPoint','weight','remark','vehicleIn','vehicleNumber','vehicleOut'];
  const labelMap: Record<string,string> = { date:'Date', location:'Location', plant:'Plant', cfa:'CFA', weight:'Weight', sto:'STO', loadingPoint:'Loading Point', vehicleIn:'Vehicle In', vehicleNumber:'Vehicle Number', vehicleOut:'Vehicle Out', slipNumber:'Slip Number', demandDate:'Demand Date', requiredDate:'Required Date', remark:'Remark' };
  const previewRows = preview.rows.slice(0, 200);
  return <div className="bulk-modal">
    <div className="bulk-help">Paste tab-separated Excel data below. Header names are detected automatically and column order can be different. Empty cells are preserved.</div>
    <textarea className="bulk-paste-area" value={text} onChange={(e) => setText(e.target.value)} placeholder={kind === 'planning' ? 'Paste your Page 1 Excel rows here…' : 'Paste your Page 2 Excel rows here…'} spellCheck={false} />
    {preview.totalRows > 0 && <div className="bulk-summary"><span>Total Rows: <strong>{preview.totalRows}</strong></span><span>Valid Rows: <strong>{preview.validRows}</strong></span><span>Invalid Rows: <strong>{preview.invalidRows}</strong></span><span>Duplicates: <strong>{preview.duplicateRows}</strong></span></div>}
    {preview.totalRows > 0 && <div className="bulk-preview"><table><thead><tr><th>Row</th>{schemaKeys.map((key) => <th key={key}>{labelMap[key]}</th>)}<th>Validation</th></tr></thead><tbody>{previewRows.map((row) => <tr key={row.rowNumber}><td>{row.rowNumber}</td>{schemaKeys.map((key) => <td key={key}>{row.mapped[key] || '-'}</td>)}<td><span className={`bulk-status ${row.status}`}>{row.status === 'valid' ? '✓ Valid' : row.status === 'duplicate' ? '⚠ Duplicate' : '⚠ Invalid'}</span>{row.reasons.length > 0 && <div className="bulk-reasons">{row.reasons.join('; ')}</div>}</td></tr>)}</tbody></table>{preview.totalRows > 200 && <div className="table-note">Showing first 200 preview rows. All rows are parsed and validated; this display limit does not limit import.</div>}</div>}
    {!headers.length && text && <div className="bulk-empty-state">Paste a header row followed by tab-separated data rows.</div>}
    <div className="bulk-actions"><button onClick={() => setText('')}>Clear</button><button onClick={onClose}>Cancel</button><button className="primary" disabled={!preview.validRows} onClick={() => onImport(preview)}>Import Valid Rows</button></div>
  </div>;
}
